...
public class CheckMailServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String code = request.getParameter("txtMailCode");
        HttpSession session = request.getSession();
        TblUserDTO user = (TblUserDTO) session.getAttribute("USER");
        String url = HOME_PAGE;
        try {
            TblUserDAO dao = new TblUserDAO();
            if(code.equals(dao.getCode(user.getUsername()))){
                dao.updateAccount(user.getUsername(), true);
                session.removeAttribute("VERIFY");
            } else
            {
                request.setAttribute("FAILVERIFY", "fail");
            }
            
        } catch (SQLException e) {
            log("CheckMailServlet_SQLex: " + e.getMessage());
        } catch (NamingException e) {
            log("CheckMailServlet_NamingEx: " + e.getMessage());
        } finally {
